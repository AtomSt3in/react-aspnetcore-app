...
export default function MappingStep({ sampleRow, mapping, setMapping, suggestedMapping, autoMap, onNext, onBack }: Props) {
  ...
  const applySuggested = () => {
    ...
    columns.forEach((c) => {
      merged[c] = suggestedMapping[c] ?? mapping[c] ?? null;
    });
    setMapping(merged);
  };

  const unmappedCount = useMemo(() => {
    return columns.filter(c => !mapping[c]).length;
  }, [columns, mapping]);

  return (
    <div>
      <p className="text-muted">Mapea las columnas de tu archivo a los campos esperados por el sistema. Usa "Auto map" para sugerencias rápidas.</p>

      <div className="mb-3">
        <Button color="outline-primary" onClick={() => applySuggested()} className="me-2">Auto map</Button>
        <Button color="outline-secondary" onClick={() => setMapping(columns.reduce((acc, c) => ({ ...acc, [c]: null }), {} as Mapping))}>Reset</Button>
        <span className="text-muted ms-3">Columnas sin mapear: {unmappedCount}</span>
      </div>
      ...
      {columns.map(col => (
        <Row className="g-2 mb-2" key={col}>
          <Col md={6}>
            ...
            <div className="text-muted small">Ej: {String(sampleRow![col])}</div>
          </Col>
          <Col md={6}>
            <FormGroup>
